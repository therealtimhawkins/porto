...
const run = async (searchTerm = process.argv[2] || "") => {
  ...
  if (parsedStdout.length) {
    if (parsedStdout.length === 1) {
      console.log(
        chalk.cyan(`🕵️‍♂️  Found ${chalk.magenta(parsedStdout.length)} process...`)
      )
    } else {
      console.log(
        chalk.cyan(
          `🕵️‍♂️  Found ${chalk.magenta(parsedStdout.length)} processes...`
        )
      )
    }
    ...
    console.log(
      chalk.cyan(
        `🔫 Killed process ${chalk.magenta(
          process.procname
        )} on port ${chalk.magenta(process.port)}`
      )
    )
  } else {
